 ...
 * @brief Main program of GTK+ electrotest GUI application.
 */
...
/*! UserInput struct contains user input data
 *
 * gtk_spin_buttons[4]  input fileds from user
 * button_parallel      a toggle button indicating parallel or serial circuit
 * buffer               a textbuffer for the output to the user
 */
typedef struct UserInput {
   GtkSpinButton *gtk_spin_buttons[4]; /*! Spin buttons */
   GtkToggleButton *button_parallel;   /*! Parallel or serial indicator */
   GtkTextBuffer *buffer;              /*! text buffer for output */
} UserInput;

/*! Callback for computing electrotest given user input
 * \param[in] widget Unused
 * \param[in] data   Contains user input (see UserInput struct)
 */
...
static void linumtest_callback(GtkWidget *widget, gpointer data)
{
	GdkColor color;

	gboolean isTrue = bTestElectricFunctions(TRUE);
          ...
          char *argv[] )
{
   ...
   button_linumtest = gtk_button_new_with_label ("Test");
   ...
   g_signal_connect (button_compute, "clicked",
                     ...
                     user_input);
   g_signal_connect (button_linumtest, "clicked",
                      G_CALLBACK(linumtest_callback),
                      button_linumtest);
   ...
   gtk_table_attach(GTK_TABLE(table), button_linumtest, 0, 2, 5, 6, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK,5, 5);
